...
| 🔗 友链管理 | 友情链接的展示与后台管理 |
| 👤 个人资料 | 关于页 / 个人主页配置 |
| 🔄 Gitee 同步 | 一键把博客数据备份到自己的 Gitee 仓库；内置「零门槛 · 新手向导」教你从零连接 Gitee（向导第 4 步测试通过即自动保存配置） |
| 🪟 窗口记忆 | 桌面版记住你最近一次调整的窗口大小，下次打开自动恢复（支持边缘拖拽调整） |
| 🔗 友链增强 | 友链支持本地上传头像/图标、排序权重，随云同步一起备份 |
...
个人博客/
├── 项目源码/                     # 博客系统完整源码
│   ├── app.py                    # 主应用：路由 / 数据库 / 页面渲染（约 2550 行）
